## 문제 링크
[코드트리 | 코딩테스트 준비를 위한 알고리즘 정석](https://www.codetree.ai/training-field/frequent-problems/problems/battle-ground/description)

## 문제 풀이

1. 문제에 기술되어 있는대로 로직을 구현하면 된다.
2. 특이점은 heapq를 이용해서 가장 강한 총을 빠르게 구할 수 있게 한다.
3. 오류를 최소화하기 위해 문제에 주어진 변수들을 최대한 분리한다.
4. 예를 들어 플레이어의 정보에 위치를 저장하고 따로 지도 배열에 플레이어 아이디를 저장한다.
5. 플레이어가 이동했을 때 해당 좌표에 이미 다른 플레이어가 있다면 지도에 플레이어 아이디 저장을 보류하고 싸움을 하면 된다.
6. 싸움 후에 점수 계산과 총 획득, 내려놓기, 위치 저장을 하면 된다.
