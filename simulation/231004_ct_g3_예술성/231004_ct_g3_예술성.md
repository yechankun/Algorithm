...
[코드트리 | 코딩테스트 준비를 위한 알고리즘 정석](https://www.codetree.ai/training-field/frequent-problems/problems/artistry/description)

## 문제 풀이

1. BFS와 시뮬레이션으로 푼다.
2. 전체 보드를 탐색해가며 점수를 계산한다.
3. 점수 계산법은 우선 그룹을 나눈다.
4. 그룹별로 그룹번호가 지정되지 않은 모서리가 겹친 칸을 저장한다.
5. 그렇게 하면 현재 그룹보다 그룹번호가 큰 모서리가 겹친 칸만 저장되어 한 번만 순회할 수 있다.
6. 저장한 모서리 칸을 순회해서 점수를 계산한다.
7. 문제에 주어진대로 회전한다.
8. 이를 주어진 횟수만큼 반복한다.
